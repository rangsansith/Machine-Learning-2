...
public class RegressionTree {
	...
	private static Double[] FindBestSplit(ArrayList<Double> labels,ArrayList<ArrayList<Double>> feats){
		...
		double re=CalculateMSE(labels)-2500,curfeat=-1.0,tempre=0.0,thresh=-1.0,tempthresh=0.0;
	...
	private static Double[] CreateFeatMat(ArrayList<Double> labels,ArrayList<Integer> index,ArrayList<ArrayList<Double>> feats){
		...
		System.out.println(CalculateMSE(labelsnew)+"inga");
	...
	private static CustomTree BuildTree(ArrayList<ArrayList<Double>> features,ArrayList<Double> labels,HashMap<Integer, ArrayList<Integer>> indexlist){
		...
		for (int i = 0; i < 7; i++) {
			...
			CustomTree node = new CustomTree();
			System.out.println(nodeno+"   "+indexlist.get(nodeno).size());
	...
	public static void main(String[] args) {
		...
		//Double[] all=FindBestSplit(labelstrain, featurestrain);//SelectThreshold(labelstrain, featurestrain.get(0));
		//System.out.println(all[0]+"    "+all[1]);
		CustomTree tree = BuildTree(featurestrain, labelstrain, indexlist);
		System.out.println("Average Testing:"+AccuracyPredict(featurestest, labelstest, tree)[0]);
		System.out.println("Average Training:"+AccuracyPredict(featurestrain, labelstrain, tree)[0]);
	}
