...
public class TwoClassAll {
	...
	static double sigmag=1/featlen;	
	static double tanc=3;
	...
	private static double euclidist(int i, int j) {
		double sum=0;
		for (int k=0;k<featlen;k++) {
			sum+=(featurestrain[i][k]-featurestrain[j][k])*(featurestrain[i][k]-featurestrain[i][k]);
		}
		return sum;
	}
	private static double kernel(int i,int j){
		if(kerneltype==1)
		{
		...
		{
         return Math.exp(-sigmag*sigmag*euclidist(i, j));
		}
		...
		{
			return Math.pow(sigmag*dotproduct(i, j)+b, tanc);
		}
	}
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < testlen; i++) {
			//System.out.println(mpredict(i)+" "+labelstest[i]);
			if(mpredict(i)*labelstest[i]>0)
				acc++;

		}
		System.out.println(acc/(double) testlen);
		double[] weights=new double[featlen];

		for (int j = 0; j < featlen; j++) {
			weights[j]=0;
			for (int i = 0; i < trainlen; i++) {
				weights[j]+=alpha[i]*labelstrain[i]*featurestrain[i][j];
			}
		}
		acc=0.0;
		for (int i = 0; i < trainlen; i++) {
			double temp=b;
			for (int j = 0; j < featlen; j++) {
				temp=weights[j]*featurestrain[i][j];
			}
			//System.out.println(temp+" "+labelstrain[i]+" "+mpredict(i));
			if(temp*labelstrain[i]>0)
				acc++;
